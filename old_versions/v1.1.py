...
class Minesweeper:
    ...
    def __init__(self, tk):
        ...
        self.message_label.grid(row=self.size + 2, columnspan=self.size,)

        # Create a button to restart the game
        self.restart_button = Button(self.frame,
                                     text="Restart", command=self.restart)
        self.restart_button.grid(row=self.size + 3,
                                 column=0, columnspan=9, sticky=W)

        # Create a button to reload the game
        self.reload_button = Button(self.frame,
                                    text="Reload", command=self.reload)
        self.reload_button.grid(row=self.size + 3, column=(self.size // 2) - 1,
                                columnspan=8, sticky=W)

        # Create a button to quit the game
        self.quit_button = Button(self.frame,
                                  text="Quit", command=self.tk.destroy)
        self.quit_button.grid(row=self.size + 3,
                              column=self.size - 3, columnspan=6, sticky=E)
